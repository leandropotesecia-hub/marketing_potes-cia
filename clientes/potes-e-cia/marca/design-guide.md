# Guia de Design — Potes & Cia

## Cores
- **Fundo principal:** azul-marinho / chumbo escuro
- **Cor de destaque / CTA:** amarelo-mostarda (usado em preços, badges de desconto, selos "OFF")
- **Texto principal:** branco (sobre fundo escuro); preto em elementos sobre fundo claro pontual
- **Cor proibida:** nenhuma restrição definida ainda

## Tipografia
- **Títulos e destaques:** sans-serif bold e condensada, de alto impacto (estilo cartaz/campanha) — ex: "COPO ROCKS", "FAMÍLIA KALEIDO"
- **Corpo:** sans-serif neutra, legibilidade em primeiro lugar
- **Toque premium:** linhas mais sofisticadas do catálogo (ex: Grandeur, Pinch Grandeur, Pianezza) usam serifa/script elegante em dourado ou bordô, contrastando com o restante bold/comercial

## Estilo geral
Comercial, confiante e chamativo — preço e oferta sempre em destaque, com tom levemente comemorativo em marcos da marca. Nas linhas premium, ganha um toque de sofisticação e elegância.

## Composição de fotos
- **Cenário:** ambiente real (ex: mesa de madeira, fundo desfocado sugerindo bar/adega com garrafas) — não é fundo de estúdio/abstrato
- **Preço na arte:** aparece — badge de destaque (azul-marinho + amarelo-mostarda), formato "A PARTIR DE R$ [valor] un"
- **Chamada/CTA visual na imagem:** sim, frase de efeito grande faz parte do design (não fica só na legenda)
- **Canais de venda na arte:** não observado no post analisado (nem Mercado Livre, nem Shopee aparecem dentro da imagem)
- **Padrão exato de badges (dois badges distintos, não um só):**
  1. **Badge de referência + preço:** "Ref.: [número] - [capacidade]ml" na linha de cima, "A PARTIR DE R$ [valor] un" em destaque embaixo
  2. **Badge de quantidade por caixa:** separado do badge de referência/preço, com ícone de caixa + "Caixa c/ [quantidade]"
- **Baseado em:** 1 post real (linha Grandeur) — padrão pode não representar todas as linhas do catálogo; vale revisitar com mais posts se surgir divergência

## O que NUNCA fazer
Nenhuma restrição visual explícita definida ainda.

## Logo
- **Arquivo:** não enviado ainda
- **Onde usar:** não definido

---
## Nota de inconsistência
O site institucional (potesecia.com.br) segue uma identidade visual diferente da usada no Instagram: fundo branco/off-white, destaque em laranja/vermelho, tipografia sans-serif neutra, sensação clean e minimalista (perfil "atacado industrial B2B"). Esse guia usa o **Instagram como referência principal**, por decisão do cliente — o site fica registrado aqui como ponto de possível alinhamento futuro entre os canais.
