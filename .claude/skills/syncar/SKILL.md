...
Se `git remote get-url origin` não retornar nada, o workspace ainda não está conectado ao
GitHub.
...
> 2. Crie um repositório (pode ser privado, nome sugerido: `potes-e-cia` ou o nome do projeto)
> 3. Não inicialize com README — deixa vazio
> 4. Me passa o link do repositório criado (ex: https://github.com/seunome/potes-e-cia)"
...
> Sempre que quiser salvar, é só me pedir 'synca' (ou rodar /syncar) que eu faço o commit e o
> push."
...
Para a descrição do commit, use o que foi feito na sessão (ex: "sync: skill roteiro-post v3",
"sync: análise vendas julho vila-aromas", "sync: reorganização de pastas por mês"). Se não
souber o que colocar, use `sync: atualizações do dia`.
